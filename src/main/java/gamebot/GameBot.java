...
public class GameBot {
	...
	public void init(String[] args) {
		...
		buildReadyEvent()
				.then(Mono.when(buildReadyEvent(), buildMemberJoinEvent(), buildMessageCreateEvent(),
						buildMessageUpdateEvent(), buildChatInputInteractionEvent(), MeetupEventManager.init(),
						buildTimedInterval(admin), SpotifyHelpers.init(args[1], args[2])))
	...
	private Mono<?> buildReadyEvent() {
		return gateway.on(GuildCreateEvent.class).next().flatMap(ready -> {
			return ChannelLogger.init(ready.getGuild())
					.then(Mono.when(Flux.fromIterable(listeners).flatMap(element -> element.onReady(ready))));
		}).onErrorResume(t -> {
			t.printStackTrace();
			return Mono.empty();
		});
