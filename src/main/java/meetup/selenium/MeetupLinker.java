...
public class MeetupLinker {
	...
	public static Mono<Void> readVerified() {
		verified.clear();
		return RedisConnector.cacheFile(new File("verified"), key)
				.then(RedisConnector.readAllFields(key).flatMap(map -> {
					verified = map;
					return Mono.empty();
				})).then();
	}
	...
	public static Optional<String> getUserByMeetupId(String id) {
		return verified.entrySet().stream().filter(entry -> id.equals(entry.getValue())).map(entry -> entry.getKey())
				.findFirst();
	}
